...
TEMPLATE_TEST_CASE_SIG("DenseMatrixBlockIterable", "[DenseMatrixBlockIterable]", ((int Ordering, int BR, int BC, int NBR, int NBC), Ordering, BR, BC, NBR, NBC),
...
TEMPLATE_TEST_CASE_SIG("DenseMatrixBlockIterable-RowMajor", "[DenseMatrixBlockIterable]", ((int BR, int BC, int NBR, int NBC), BR, BC, NBR, NBC),
  (2, 3, 3, 4),
  (2, 3, 3, Eigen::Dynamic),
  (2, 3, Eigen::Dynamic, 4),
  ...
  for (int r = 0; r < mat.numBlocksRow(); r++) {
    for (int c = 0; c < mat.numBlocksCol(); c++) {
      if (mat.hasBlock(r, c)) {
        mat.block(r, c).setConstant(r + c / 10.0);
      }
    }
  }
  ...
  for (int r = 0; r < mat.numBlocksRow(); r++) {
    for (int c = 0; c < mat.numBlocksCol(); c++) {
      if (mat.hasBlock(r, c)) {
        REQUIRE((mat.block(r, c).array() == r + c / 10.0).all());
      }
    }
  }
...
TEMPLATE_TEST_CASE_SIG("DenseMatrixBlockIterable-ColMajor", "[DenseMatrixBlockIterable]", ((int BR, int BC, int NBR, int NBC), BR, BC, NBR, NBC),
  (2, 3, 3, 4),
  (2, 3, 3, Eigen::Dynamic),
  (2, 3, Eigen::Dynamic, 4),
  (2, 3, Eigen::Dynamic, Eigen::Dynamic),
